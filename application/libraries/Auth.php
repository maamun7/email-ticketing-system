...
class Auth {
	...
	{   
        if($url==''){$url = base_url().'dashboard/login';}
		$CI =& get_instance();
    ...
    function check_permission($slug, $redirect=true, $user_id = 0)
    {
        ...
        if( count($res) > 0){
            ...
            if( $redirect )
            {
                $CI->session->set_userdata(array('error_message'=>"You are not authorized to view this page!"));
                redirect(base_url().'dashboard','refresh');
            }
            else
                return false;
            //$CI->output->set_header("Location: ".base_url().'dashboard', TRUE, 302);
        }
    }
